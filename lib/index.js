$(document).ready(function(){
	...
	$.when(
		...
		$.ajax('adat/utoirat.html')
			...
			app.config.door = $.trim(door);
			app.config.message = $.trim(message);
			app.config.newsletter = $.trim(newsletter);
			app.config.password = $.trim(password);
			app.config.order = $.trim(order);
			app.config.ps = $.trim(ps);
		}).always(function () {
			app.ready.resolve();
	});
